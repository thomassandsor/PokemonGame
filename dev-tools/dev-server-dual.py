...
def main():
    ...
    print(f"🎮 Pokemon Game Smart Development Server")
    print(f"📁 Serving from: {project_root}")
    print(f"🌐 URL: http://localhost:{PORT}")
    print(f"� Auto-detecting API configuration...")
    print()
    
    # Auto-detect API configuration
    detected_mode, api_target = detect_api_configuration()
    
    print()
    print(f"�🔄 API Mode: {detected_mode}")
    print(f"📡 API Target: {api_target}")
    
    if detected_mode == "LOCAL":
        local_running = check_local_functions_running()
        if local_running:
            print(f"✅ Local Azure Functions are running")
        else:
            print(f"⚠️  Local Azure Functions not detected on {LOCAL_FUNCTIONS_URL}")
            print(f"🔄 Will fallback to live Azure when needed")
            print(f"💡 To start local functions: cd api && func start --port 7071")
    
    print(f"⚡ Live reload: Just refresh your browser after changes!")
    print()
    
    with socketserver.TCPServer(("", PORT), PokemonDualDevHandler) as httpd:
